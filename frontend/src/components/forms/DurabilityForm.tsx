...
export default function DurabilityForm({ sessionId, results }: Props) {
  const queryClient = useQueryClient();
  const [rows, setRows] = useState<Row[]>([
    { key: 1, test_point_load: '', indicated_value: '', correction: '0' },
  ]);

  const durResults = (results ?? []).filter((r) => r.test_type === 'durability');
  ...
  const saveObservations = () => {
    const obs = rows.filter((r) => r.test_point_load && r.indicated_value).map((r, i) => ({
      test_type: 'durability' as const, test_point_load: r.test_point_load, indicated_value: r.indicated_value, correction: r.correction || '0', trial_number: i + 1,
    }));
  ...
  return (
    <div>
      <p style={{ fontSize: 12, color: '#999999', marginBottom: 16 }}>
        Only for instruments with Max &le; 100 kg. Record weighing performance after 100,000 load cycles at ~0.5 x Max.
      </p>
      ...
      {durResults.length > 0 && (
        <div style={{ marginTop: 32 }}>
          ...
          <Table dataSource={durResults} columns={[
