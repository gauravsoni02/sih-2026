...
export default function TiltForm({ sessionId, results }: Props) {
  const queryClient = useQueryClient();
  const [rows, setRows] = useState<Row[]>([
    { key: 1, test_point_load: '', indicated_value: '', correction: '0' },
  ]);

  const tiltResults = (results ?? []).filter((r) => r.test_type === 'tilt');
  ...
  const saveObservations = () => {
    const obs = rows.filter((r) => r.test_point_load && r.indicated_value).map((r, i) => ({
      test_type: 'tilt' as const, test_point_load: r.test_point_load, indicated_value: r.indicated_value, correction: r.correction || '0', trial_number: i + 1,
    }));
  ...
  return (
    <div>
      <p style={{ fontSize: 12, color: '#999999', marginBottom: 16 }}>
        Not applicable for Class I instruments. Record readings with instrument tilted per Section 3.9.1.
      </p>
