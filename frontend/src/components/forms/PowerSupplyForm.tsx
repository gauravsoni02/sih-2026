...
export default function PowerSupplyForm({ sessionId, results }: Props) {
  const queryClient = useQueryClient();
  const [rows, setRows] = useState<Row[]>([
    { key: 1, test_point_load: '', indicated_value: '', correction: '0' },
  ]);

  const psResults = (results ?? []).filter((r) => r.test_type === 'power_supply');
  ...
  const saveObservations = () => {
    const obs = rows.filter((r) => r.test_point_load && r.indicated_value).map((r, i) => ({
      test_type: 'power_supply' as const, test_point_load: r.test_point_load, indicated_value: r.indicated_value, correction: r.correction || '0', trial_number: i + 1,
    }));
  ...
  return (
    <div>
      <p style={{ fontSize: 12, color: '#999999', marginBottom: 16 }}>
        AC mains: test at 0.85 x Unom and 1.10 x Unom. Record weighing performance at each voltage.
      </p>
      ...
      {psResults.length > 0 && (
        <div style={{ marginTop: 32 }}>
          ...
          <Table dataSource={psResults} columns={[
